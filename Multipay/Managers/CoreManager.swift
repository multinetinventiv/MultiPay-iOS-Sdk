...
internal class CoreManager {
    ...
    {
        didSet{
            NotificationCenter.default.post(name: NSNotification.Name(rawValue: Constants.Notifications.AvatarLoadedNotification), object: nil)
        }
    }
    ...
    var token:String? {
        didSet{
            saveToNSUserDefaults("token", value: token!)
        }
    }
    ...
    func lookUpTableReset() {
        removeDataWithKey(LocalStorageKeys.kLookUp)
        self.getLookUpTable(callback: nil, errorCallback: nil)
    }
    
    func lookUpIsExist() -> Bool{
        
        let lookUp = loadObjectFromUserDefaults(LocalStorageKeys.kLookUp)
        
        if lookUp == nil || lookUpTableIsChanged(newVersion: self.setting.lookUpListVersion){
            //log.debug("**********LOCAL LOOKUP VERSION ********** \(loadFromNSUserDefaults(LocalStorageKeys.kLookUpVersion))")
            //log.debug("**********NEW LOOKUP VERSION ********** \(self.setting.lookUpListVersion)")
            return false
        }
        
        if LookUp.sharedInstance.lookUpTable.isEmpty {
            
            LookUp.sharedInstance.lookUpTable = LookUpTable.createWithDictionary(lookUp as? [[String : AnyObject]])
        }
    ...
    private func lookUpTableIsChanged(newVersion: String?) -> Bool{
        
        //log.debug("**********LOCAL LOOKUP VERSION ********** \(loadFromNSUserDefaults(LocalStorageKeys.kLookUpVersion))")
        //log.debug("**********NEW LOOKUP VERSION ********** \(newVersion)")
        
        if(newVersion == nil ) {
            return false
        }
        
        if let ver = loadFromNSUserDefaults(LocalStorageKeys.kLookUpVersion){
    ...
    func getLookUpTable(callback: lookUpCallBack?, errorCallback: errorCallBack?)
    {
        ...
        if lookUpIsExist() {
            ...
            if callback != nil {
                ...
                let _ = callback!(LookUp.sharedInstance.getLookUpTable())
            }
            
            return
        }
        
        let parameters = [languageCodeKey :  CoreManager.Instance().getLanguageRegion(),
                          appTokenKey     :  ServiceUrl.getToken()]
        
        let url = ServiceUrl.getURL(ServiceConstants.ServiceName.SPFilterService)
        
        let httpHeaders = HTTPHeaders(Auth.shared.getHeader())

        manager?.request(url, method: .post, parameters: parameters, encoding: JSONEncoding.default, headers: httpHeaders).responseJSON { response in
            ...
            guard response.error == nil else {
                ...
                {
                    errorCallback!(errorModel as! ErrorModel)
                }
                return
            }
            ...
            case .success(let value):
                    ...
                    if  let jsonData = json.dictionaryObject,let resultDictionary = jsonData[resultKey] {
                        ...
                        guard let _ = theCode, theCode as! Int == 0 else {
                            ...
                            let errorModel = ErrorModel(result: String(describing: theCode),
                                                        code: String(describing: theCode),
                                                        message: jsonData[resultMessageKey] as? String,
                                                        additionalData: nil)
                            if errorCallback != nil{
                        ...
                        LookUp.sharedInstance.lookUpTable = LookUpTable.createWithDictionary(resultDictionary as? [[String : AnyObject]])
                        saveObjectToNSUserDefaults(LocalStorageKeys.kLookUp, value: resultDictionary as AnyObject)
                        saveToNSUserDefaults(LocalStorageKeys.kLookUpVersion, value: self.setting.lookUpListVersion ?? "")
                        
                        log.debug("LookUpTable: \(resultDictionary)")
                        ...
                        let errorModel = ErrorModel(result: "111",
                                                    code: "111",
                                                    message: "User Card Json error",
                                                    additionalData: nil)
            ...
            case .success(let value):
                ...
                if  let jsonData = json.dictionaryObject,let resultDictionary = jsonData[resultKey] {
                    ...
                    if  ServiceConstants.ErrorCheck.isUserAggrement((theCode as! Int)) {
                        NotificationCenter.default.post(name: NSNotification.Name(rawValue: Constants.Notifications.UserAggrementNotification), object: nil)
                        return
                    }
                    
                    if ServiceConstants.ErrorCheck.isKVKKAgreementErrorEquals(to: theCode as! Int) {
                        NotificationCenter.default.post(name: NSNotification.Name(Constants.Notifications.KVKKAggrementNotification), object: nil)
                        return
                    }
                    ...
                    NotificationCenter.default.post(name: NSNotification.Name(rawValue: Constants.Notifications.UserLoadedNotification), object: nil)
                }else{
    ...
    func clear()  {
        ...
        removeDataWithKey(LocalStorageKeys.kCardTotalBalanceChoice)
        CoreManager.Instance().lookUpTableReset()
